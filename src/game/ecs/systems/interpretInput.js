...
export const interpretInputSystem = {
    ...
    update: function (world) {
        ...
        if (mouseLeftDown) {
            if (!InputState.isDragging[playerEid]) {
                const d = engineDistanceClass.BetweenPoints(mousePosition, downPosition)
                if (d && d >= this.minimalDistance) {
                    InputState.isDragging[playerEid] = true;
                }
            }
        }
        if (InputState.isDragging[playerEid]) {
            rect = this.getRect(mousePosition, downPosition)
        ...
        for (const eid of bitEcs.query(world, [Interactable, Transform])) {
            if (!InputState.isDragging[playerEid]) {
                const doesIntrsctPoint = this.intersectPoint(world, eid, mousePosition.x, mousePosition.y);
                if (doesIntrsctPoint) {
    ...
    getRect: function (mousePosition, downPosition) {
    ...
    intersectRect: function (world, eid, rectX, rectY, rectWidth, rectHeight) {
        ...
        const doesContain = (bitEcs.hasComponent(world, eid, Interactable) && this.rectVar && this.rectVar.contains(x, y))
        return doesContain
    },
